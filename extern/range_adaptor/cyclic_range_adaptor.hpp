...
public:
    template <class Cutoff>
    cyclic_range_adaptor(Iterator begin, Iterator end, Cutoff cutoff)
        : begin_(begin), end_(end), cutoff_(tlx::round_up_to_power_of_two(cutoff)) {}

    template <class Range, class Cutoff>
    cyclic_range_adaptor(Range&& range, Cutoff cutoff)
        : begin_(range.begin()), end_(range.end()), cutoff_(tlx::round_up_to_power_of_two(cutoff)) {}
