...
public:
    ...
    static void all_to_all(
        std::vector<std::vector<T>> const&    send_buf,
        ...
        auto probe = [&]() {
            ...
            while (iprobe_success > 0) {
                ...
                if (iprobe_success > 0) {
                    ...
                    MPI_Status rst;
                    MPI_Recv(
                        recv_buffers[status.MPI_SOURCE].data(),
                        message_length,
                        mpi_type,
                        status.MPI_SOURCE,
                        status.MPI_TAG,
    ...
    static void sparse_all_to_all(
        const google::dense_hash_map<PEID, std::vector<T>>& send_buffers,
    ...
    template <class T>
    static void sparse_all_to_all(
        google::dense_hash_map<PEID, std::vector<T>>& send_buffers,
        google::dense_hash_map<PEID, std::vector<T>>& recv_buffers,
        MPI_Datatype                                  mpi_type,
        PEID                                          rank,
        PEID                                          size,
    ...
    static void all_to_all(
        ...
        for (int pe = 0; pe < size; ++pe) {
            if (recv_counts[pe] > 0) {
                recv_buffers[pe].insert(
                    recv_buffers[pe].end(),
                    recv_buffer.begin() + recv_displs[pe],
    ...
    template <typename T>
    static std::pair<std::vector<T>, std::vector<int>>
    all_gather(std::vector<T>& send_buffer, MPI_Datatype mpi_type, MPI_Comm comm, PEID rank, PEID size) {
        (void)rank;
    ...
    gather(Container& send_buffer, MPI_Datatype mpi_type, MPI_Comm comm, PEID root, PEID rank, PEID size) {
        int              send_count = send_buffer.size();
