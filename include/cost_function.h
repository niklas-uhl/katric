...
public:
    ...
    static void init_all(Graph& G, cetric::profiling::MessageStatistics& stats) {
        ...
        if (!G.get_graph_payload().ghost_outdegree_available) {
            comm.get_ghost_outdegree([&](Edge e) { return G.is_outgoing(e); },
    ...
    explicit CostFunction(Graph& G, const std::string& name, CostFunctionType&& cost_function)
        : G(G), global_to_index(G.local_node_count()), cost(G.local_node_count()), name(name) {
    ...
    typename std::enable_if<payload_has_outdegree<typename Graph::payload_type>::value, Degree>::type out_degree(
        NodeId local_node_id) {
        DEBUG_ASSERT(G.is_local_from_local(local_node_id) || G.get_graph_payload().ghost_outdegree_available,
                     debug_module{}, name.c_str());
...
struct CostFunctionRegistry {
    static CostFunction<Graph> get(const std::string& name,
        ...
        std::unordered_map<std::string, std::pair<std::function<void(GraphType, cetric::profiling::MessageStatistics&)>,
        ...
        G.check_consistency();
        init(G, stats);
        return CostFunction<Graph>(G, name, cost);
    }
};
