...
TEST_F(CommunicatorTest, AllGatherRegularWorks) {
    ...
    auto [recv_buffer, displs] = CommunicationUtility::all_gather(values, MPI_INT, MPI_COMM_WORLD, rank, size);
...
TEST_F(CommunicatorTest, AllGatherIrregularWorks) {
    ...
    auto [recv_buffer, displs] = CommunicationUtility::all_gather(
        values, MPI_INT, MPI_COMM_WORLD, rank, size);
    ASSERT_EQ(recv_buffer, result);
}
